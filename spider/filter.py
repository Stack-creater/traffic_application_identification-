...
ffile_name = "ojh_data/"
# pcap_name = '07_12_10_45.pcap'#文件名
# label_file = './' + pcap_name.replace('.pcap', '.csv')
# pcap_file = './' + pcap_name
data_dir = './app_ojh_01/'

for filename in os.listdir(ffile_name):
    # 拼接文件的完整路径
    filepath = os.path.join(ffile_name, filename)

    # 判断路径是否为文件且后缀为.pcap
    if os.path.isfile(filepath) and filename.endswith('.pcap'):
        # 输出文件名
        pcap_name = ffile_name + filename  # 文件名
        print(pcap_name)
        label_file = './' + pcap_name.replace('.pcap', '.csv')
        pcap_file = './' + pcap_name
